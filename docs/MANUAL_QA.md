...
5. Run `AI Commit: Set OpenRouter API Token` and enter a valid token.

## Core Flow

- Run `AI Commit: Generate Commit Message` from the Command Palette.
...
- Enable `aiCommit.autoPushAfterCommit`, regenerate, and confirm `Commit and Push` is available only when a remote/branch can be pushed.

## Safety Checks

- Remove the OpenRouter token and confirm generation offers token setup guidance.
- Use an invalid token and confirm the OpenRouter error is shown in plain language with a retry action.
- Create a large diff above `aiCommit.maxDiffCharacters` and confirm the review panel shows a truncation warning.
