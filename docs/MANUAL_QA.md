...
5. Run `CommitCraft: Set OpenRouter API Token` and enter a valid token.

## Core Flow

- Run `CommitCraft: Generate Smart Git Commit` from the Command Palette.
...
- Enable `commitCraft.autoPushAfterCommit`, regenerate, and confirm `Commit and Push` is available only when a remote/branch can be pushed.
